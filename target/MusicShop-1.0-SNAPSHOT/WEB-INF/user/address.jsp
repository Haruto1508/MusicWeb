...
function validateName(name) {
    if (!name || !name.trim()) return "Please enter full name.";
    if (!/^[\p{L}\s'.-]{2,50}$/u.test(name.trim())) return "Full name must be 2-50 characters, letters only.";
    return "";
}

function validatePhone(phone) {
    if (!phone || !phone.trim()) return "Please enter phone number.";
    if (!/^0[2-9]\d{8,9}$/.test(phone)) return "Phone number must be 10-11 digits, starting with 03, 05, 07, 08, or 09.";
    return "";
}

function validateStreet(street) {
    if (!street || !street.trim()) return "Please enter street and house number.";
    if (!/^[\p{L}\p{N}\s,.\-\/]{5,100}$/u.test(street.trim())) return "Street must be 5-100 characters, valid characters only.";
    return "";
}

function validateWard(ward) {
    if (ward && !/^[\p{L}\p{N}\s\-\/]{2,50}$/u.test(ward.trim())) return "Ward/commune must be 2-50 characters.";
    return "";
}

function validateDistrict(district) {
    if (district && !/^[\p{L}\p{N}\s\-\/]{2,50}$/u.test(district.trim())) return "District must be 2-50 characters.";
    return "";
}

function validateCity(city) {
    if (!city) return "Please select city/province.";
    if (!vietnamProvinces.includes(city)) return "Please select a valid city/province.";
    return "";
}
